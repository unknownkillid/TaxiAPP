...
async function fetch_game(req, res) {
    try {
        ...
        // If no steam_id or steam_ids are provided, return an error messag        
        ...
        if (steam_ids == undefined) {
            const response = await axios.get(`https://store.steampowered.com/api/appdetails?appids=${320}`);
            let game = response.data[320];
            return res.json({ msg: "games data", game });
        }
